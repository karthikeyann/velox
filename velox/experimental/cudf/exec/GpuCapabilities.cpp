...
uint64_t partitionedGroupbyMinGroups(uint64_t fallback) {
  ...
  // operator's state would crowd out the rest of the query rather than at it.
  //
  // The threshold is a group count because that is what the operator can check
  // for free - the state's row count - whereas its byte size would have to be
  // walked. A group carries its keys and one accumulator per aggregate; a
  // tenth of the device at an assumed hundred bytes per group puts the
  // crossover in the tens of millions of groups on a 48 GiB card, which is far
  // above any aggregation that was never at risk.
  const auto budget = fractionOfDevice(0.10);
  if (budget == 0) {
    return fallback;
  }
  static constexpr uint64_t kApproxBytesPerGroup = 100;
  return budget / kApproxBytesPerGroup;
}
