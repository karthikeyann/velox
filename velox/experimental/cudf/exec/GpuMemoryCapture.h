...
/// Returns the most recently written raw capture path, or an empty string.
std::string gpuMemoryCaptureLastPath();

/// Adds a marker to both the raw capture and an enabled Perfetto trace.
...
/// Returns Unix-epoch nanoseconds used to anchor replay into Quent time.
