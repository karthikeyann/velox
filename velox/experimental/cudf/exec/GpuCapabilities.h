...
///
/// Merging grows a single table, and the merge itself needs the old state, the
/// concatenated copy and the result at once. Partitioning makes that peak a
/// function of one partition rather than the whole state, which only pays for
/// its extra bookkeeping once the state is a material fraction of the device.
